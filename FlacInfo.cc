					...
					do
					{
						...
						case FLAC__METADATA_TYPE_PICTURE:
							...
							if (m_PictureBlock->is_valid())
								m_CoverArt.SetArt(m_PictureBlock->get_data(), m_PictureBlock->get_data_length(), true, m_PictureBlock->get_mime_type());
							break;
#endif
						}
					} while (Iterator.next());
...
void CFlacInfo::SetPictureBlock(const CCoverArt &CoverArt)
{
	if (m_PictureBlock)
	{
		...
		m_PictureBlock->set_mime_type(CoverArt.MimeType().c_str());
	}
}
